...
The following command will generate a `.github/workflows/sonar-migrate.yml` file in your project:

```bash
fluentci gh init -t sonar_pipeline
```

Or, if you already have a `.fluentci` folder (generated from `fluentci init -t sonar`) in your project:
